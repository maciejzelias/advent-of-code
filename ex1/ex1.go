...
func main() {
	...
	for _, line := range lines {
		...
		println(firstDigit)
		println(lastDigit)
		numbersCombines := firstDigit*10 + lastDigit
		println(numbersCombines)
		score += numbersCombines
	}
	print(score)
}
...
func extractByLines(input string) []string {
...
var wordDigits []WordDigit = []WordDigit{
	{"one", 1},
	{"two", 2},
	{"three", 3},
	{"four", 4},
...
func findLastDigit(input string) (int, error) {
	...
	for _, word := range wordDigits {
		if strings.LastIndex(input, word.Name) != -1 && strings.LastIndex(input, word.Name) > latestIndex {
			latestIndex = strings.LastIndex(input, word.Name)
			latestElement = word.Value
		}
	}

	if latestIndex != -1 && latestIndex > foundDigitIndex {
		return latestElement, nil
	}
	if foundDigitIndex == -1 {
		return -1, errors.New("No digit found")
	}

	return foundDigit, nil
}

func findFirstDigit(input string) (int, error) {

	foundDigitIndex := len(input)
	var foundDigit int
	for i, char := range input {
		if char >= '0' && char <= '9' {
			foundDigit = int(char - '0')
			foundDigitIndex = i
			break
		}
	}

	earliestIndex := len(input)
	var earliestElement int
	for _, word := range wordDigits {
		if strings.Index(input, word.Name) != -1 && strings.Index(input, word.Name) < earliestIndex {
			earliestIndex = strings.Index(input, word.Name)
			earliestElement = word.Value
		}
	}

	if earliestIndex != len(input) && earliestIndex < foundDigitIndex {
		return earliestElement, nil
	}
	if foundDigitIndex == len(input) {
		return -1, errors.New("No digit found")
	}
